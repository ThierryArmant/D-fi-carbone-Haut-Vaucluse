...
# 2. STYLE CSS (Glassmorphism Absolu Direct sur Colonnes et Conteneurs)
def set_style(img_b64):
    ...
    st.markdown(
        ...
        /* Force la transparence absolue de la grille Streamlit */
        .stApp, [data-testid="stMain"], .main, [data-testid="stVerticalBlock"] {{
        ...
        /* --- 💎 LE BOUCLIER GLASSMORPHISM APPLIQUÉ DIRECTEMENT AUX COLONNES --- */
        [data-testid="stColumn"], 
        div[data-testid="stExpander"],
        .stExpander,
        .glass-card {{
            background-color: rgba(13, 20, 35, 0.88) !important; /* Fond Gris Sombre Crystal */
            backdrop-filter: blur(16px) saturate(150%) !important; /* Flou dépoli ultra-pro */
            -webkit-backdrop-filter: blur(16px) saturate(150%) !important;
            border: 1px solid rgba(255, 255, 255, 0.16) !important; /* Liseré crystal protecteur */
        ...
        /* Sécurisation des polices et titres */
        ...
        h1 {{ font-size: 26px !important; font-weight: bold !important; margin: 0 !important; text-shadow: 0 2px 4px rgba(0,0,0,0.8); }}
        h2 {{ font-size: 20px !important; font-weight: bold !important; margin: 0 !important; text-shadow: 0 2px 4px rgba(0,0,0,0.8); }}
        [data-testid="stHeader"] {{ height: 0px; }}
        
        /* Éléments internes aux blocs */
...
# Rendu HTML personnalisé des barres de progression
...
with tab_dashboard:
    if not df.empty:
        ...
        # En-tête principal enveloppé dans un bandeau Crystal étanche
        st.markdown(
            """
            <div class="glass-card" style="text-align: center; margin-bottom: 15px; border-left: 5px solid #22d3ee !important;">
        ...
        # 🚀 LES DEUX BLOCS DU HAUT SONT AUTOMATIQUEMENT ENCAPSULÉS PAR LES COLONNES CRYSTAL
        ...
        # Bandeau de titre intermédiaire sécurisé
        st.markdown(
            """
            <div class="glass-card" style="text-align: center; margin-bottom: 15px; border-left: 5px solid #38bdf8 !important;">
        ...
        # 🚀 LES DEUX BLOCS DU BAS SONT ÉGALEMENT SÉCURISÉS PAR LES COLONNES CRYSTAL
        if not df_active.empty:
            ...
            with col_mid1:
                ...
                st.markdown(f'<p class="inner-title" style="color: #22d3ee; text-align: left; margin-top: 5px; margin-bottom: 15px; font-size: 18px; font-weight: bold;">🏫 Établissement : {selected_school}</p>', unsafe_allow_html=True)
...
with tab_conso_graph:
    if not df.empty:
        st.markdown(
            """
            <div class="glass-card" style="text-align: center; margin-bottom: 15px;">
        ...
        # Utilisation de colonnes simples pour héberger le graphique de manière étanche
        c_graph = st.columns(1)[0]
        with c_graph:
...
with tab_glossaire:
    st.markdown(
        """
        <div class="glass-card" style="text-align: center; margin-bottom: 15px;">
            <h2 style='color: #22d3ee; margin: 0;'>📖 Dictionnaire Carbone : Unités & Équivalents Enfants</h2>
        </div>
        """, 
        unsafe_allow_html=True
    )
    
    c_gloss = st.columns(1)[0]
    with c_gloss:
