...
def set_bg_and_style():
    st.markdown(
        ...
        /* FOND DES TABLEAUX : Blanc pur et opaque pour stopper l'image */
...
try:
    ...
    with c1:
        ...
        fig_indiv.update_layout(height=220, margin=dict(t=0, b=0, l=0, r=40), showlegend=False, coloraxis_showscale=False)
    ...
    with c2:
        ...
        if cols_valides:
            totaux = df[cols_valides].apply(lambda x: pd.to_numeric(x.astype(str).str.replace(',', '.').str.replace(r'[^\d.]', '', regex=True), errors='coerce')).sum().reset_index()
            totaux.columns = ['Poste', 'Valeur']
            fig_pie = px.pie(totaux, values='Valeur', names='Poste', hole=0.4, color_discrete_sequence=['#2ecc71', '#ff8a80', '#fbc02d', '#3498db', '#f39c12'])
            fig_pie.update_layout(height=220, margin=dict(t=0, b=0, l=0, r=0))
