...
# 2. STYLE CSS (Moteur Deep Ardoise Translucide - Calqué sur le bouton Empreinte Carbone)
def set_style(img_b64):
    ...
    st.markdown(
        ...
        /* Nettoyage des fonds opaques natifs de Streamlit */
        .stApp, [data-testid="stMain"], .main, [data-testid="stVerticalBlock"], [data-testid="stVerticalBlockContainer"] {{
            background-color: transparent !important;
        }}
        
        /* Zone centrale globale (neutre et aérée pour laisser respirer les cadres) */
        .main .block-container {{
            background-color: transparent !important;
            padding: 1rem 2rem !important;
        }}
        
        /* --- 💎 LES CADRES BLEU ARDOISE GLOSSY (Exactement le style du bouton !) --- */
        [data-testid="stColumn"], 
        div[data-testid="stBorderedContainer"],
        div[data-testid="stExpander"], 
        .stExpander,
        .glass-card {{
            background-color: rgba(30, 41, 59, 0.75) !important; /* Même teinte Ardoise Profonde que le bouton à 75% d'opacité */
            backdrop-filter: blur(16px) saturate(130%) !important; /* Flou dépoli de classe premium */
            -webkit-backdrop-filter: blur(16px) saturate(130%) !important;
            border: 1px solid rgba(255, 255, 255, 0.12) !important; /* Fine bordure crystal discrète */
        ...
        /* Bandeau de titre principal adapté */
        .main-title-box {{
            background-color: rgba(15, 23, 42, 0.85) !important;
            border-left: 5px solid #22d3ee !important;
        }}
        
        /* --- 🎛️ STYLE DES BOUTONS DE NAVIGATION DES ONGLETS --- */
        ...
        div[data-baseweb="tab"], button[data-baseweb="tab"] {{
            background-color: rgba(30, 41, 59, 0.85) !important; /* Parfaite harmonie avec les nouveaux cadres */
        ...
        /* 🛡️ SÉCURISATION DES TEXTES EN BLANC ET ARGENT POUR LE FOND SOMBRE CHIC */
        ...
        .inner-title {{ text-align: center; font-weight: bold; font-size: 16px; color: #38bdf8; margin-bottom: 12px; }}
        [data-testid="stHeader"] {{ height: 0px; }}
        
        /* Composants internes des fiches */
        .anecdote {{ background-color: rgba(30, 58, 138, 0.5); padding: 12px 14px; border-left: 4px solid #3b82f6; border-radius: 4px; color: #f1f5f9; font-size: 13px; }}
        .unit-box {{ background-color: rgba(15, 23, 42, 0.6) !important; padding: 10px; border-radius: 6px; border: 1px dashed rgba(255, 255, 255, 0.2); font-size: 13px; color: #f1f5f9; }}
        
        /* Jauges et barres ADEME */
...
with tab_dashboard:
    if not df.empty:
        ...
        # Titre enveloppé dans sa boîte ardoise signature
        with st.container(border=True):
            st.markdown('<div class="main-title-box"></div>', unsafe_allow_html=True)
        ...
        # Les fiches du haut reprennent l'effet ardoise
            ...
            with st.container(border=True):
                ...
                fig = go.Figure(go.Indicator(
                    ...
                    gauge = {
                        ...
                        'bar': {'color': "#22d3ee"}, 
                        'steps': [{'range': [0, 500], 'color': "rgba(34, 197, 94, 0.2)"}, {'range': [500, 1000], 'color': "rgba(249, 115, 22, 0.2)"}, {'range': [1000, 2000], 'color': "rgba(239, 68, 68, 0.2)"}], 
        ...
        # Fiches inférieures analytiques
        if not df_active.empty:
            ...
            with col_mid1:
                with st.container(border=True):
                    ...
                    st.markdown(f'<p class="inner-title" style="color: #38bdf8; text-align: left; margin-top: 5px; margin-bottom: 10px; font-size: 18px; font-weight: bold;">🏫 Établissement Audité : {selected_school}</p>', unsafe_allow_html=True)
            ...
            with col_mid2:
                with st.container(border=True):
                    st.markdown('<p class="inner-title" style="color: #38bdf8; text-align: left; margin-bottom: 25px; font-size: 18px; font-weight: bold;">🌍 Global : Secteurs d\'impact du Réseau</p>', unsafe_allow_html=True)
