...
# Moteur de recherche absolu pour ton fichier "image_1"
...
# 2. DESIGN BLEU ARDOISE GLOSSY UNIFIÉ (Inviolable et fonctionnel)
def set_style(img_b64):
    ...
    st.markdown(
        ...
        /* Neutralisation des voiles natifs */
        ...
        /* 💎 LE COCKPIT BLEU ARDOISE (Appliqué aux vrais cadres de mise en page) */
        div[data-testid="stBorderedContainer"],
        div[data-testid="stExpander"], 
        .stExpander,
        [data-testid="stColumn"] {{
            background-color: rgba(30, 41, 59, 0.78) !important; /* Même couleur que le bouton */
        ...
        /* Forcer les conteneurs expanders imbriqués à garder une taille normale et intégrée */
        div[data-testid="stExpander"] {{
        ...
        /* 🛡️ SÉCURISATION INTÉGRALE DES TEXTES SUR FOND ARDOISE */
        h1, h2, h3, label, .stMarkdown p, [data-testid="stWidgetLabel"] p, .pole-header, .sub-pole-header, summary {{
        ...
        .anecdote {{ background-color: rgba(30, 58, 138, 0.5); padding: 12px 14px; border-left: 4px solid #3b82f6; border-radius: 4px; color: #f1f5f9; font-size: 13px; margin-top: 8px; }}
        .unit-box {{ background-color: rgba(15, 23, 42, 0.6) !important; padding: 10px; border-radius: 6px; border: 1px dashed rgba(255, 255, 255, 0.2); font-size: 13px; color: #f1f5f9; margin-bottom: 8px; }}
...
# Fonction de dessin des barres de progression
...
# 3. INTERCONNEXION GOOGLE SHEETS
votre_gid = "169103083" 
...
with tab_dashboard:
    if not df.empty:
        ...
        if not df_active.empty:
            ...
            # 🏫 BLOC DE GAUCHE : AUDIT ÉTABLISSEMENT UNIQUE
            with col_mid1:
            ...
            # 🌍 BLOC DE DROITE : RESTAURÉ AVEC LES VRAIS EXPANDERS DE CLIC GLOBAL SYSTEM
            with col_mid2:
